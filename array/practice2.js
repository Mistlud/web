...
let data = `[{"id":1,"first_name":"Armin","last_name":"Bradforth","email":"abradforth0@state.gov","gender":"Male","ip_address":"119.31.135.166"},
...
//console.log(objAry)
let result = objAry.reduce((acc, now, idx) => {
  if (idx == 0)
    acc += '<ul>'
  console.log(acc);
  acc += `<li>${now['id']}, ${now['first_name']}</li>`
